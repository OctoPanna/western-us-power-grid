...
def cascade_failure_basic(graph, initial_failed_edges, threshold=0.5, max_seconds=300):
...
def cascade_failure_DC(G0, S_base=100.0, trip_threshold=1.0, max_iter=100, slack_node=None, failed_nodes=None):
    """
    G0: original graph
    trip_threshold: trip if |flow_MW| > capacity_MW * trip_threshold -> for tolerance
    max_iter: maximum number of cascade iterations
    slack_node: node to use as slack (if None, use first node)
    failed_nodes: list of nodes to fail at start
    ...
    if failed_nodes is not None:
        # print("Inital failures:")
        for node in failed_nodes:
            # print("node: ", node)
            # remove incident edges
            for u, v in list(G.edges(node)):
                e = tuple(sorted((u, v)))
        ...
        history.append({
                ...
                'overloaded_count': len(failed_edges),
                'overloaded_edges': [(u, v) for (u, v) in failed_edges],
                'edge_flows_MW': None
            })
        i += 1
    ...
    while i <= max_iter:
        ...
        history.append({
            ...
            'overloaded_edges': [tuple(sorted((u, v))) for (u, v, _, _) in overloaded],
        ...
        for u, v, _, _ in overloaded:
            if G.has_edge(u, v):
                failed_edges.append(tuple(sorted((u, v))))
                G.remove_edge(u, v)
